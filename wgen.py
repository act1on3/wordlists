...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-chr', '--chars',
        default=None, help='characters to iterate. You can use types: ascii_lowercase, ascii_uppercase, digits, punctuation. You can mix types - just write needed types into one line.')
    parser.add_argument(
    ...
    if 'ascii_lowercase' in args.chars:
        chars += string.ascii_lowercase
        count += 1

    if 'ascii_uppercase' in args.chars:
        chars += string.ascii_uppercase
        count += 1

    if 'digits' in args.chars:
        chars += string.digits
        count += 1
    
    if 'punctuation' in args.chars:
    ...
    createWordList(chars, args.min_length, args.max_length, args.output)
